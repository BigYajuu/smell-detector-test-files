...
class TestFormatSelection(unittest.TestCase):
    def test_prefer_free_formats(self): # Anony
    ...
    def test_format_selection(self):    # Anony
        formats = [
    ...
    def test_format_selection_audio(self):  # Anony
        formats = [
    ...
    def test_format_selection_audio_exts(self): # Anony
        formats = [
    ...
    def test_format_selection_video(self):   # Anony
        formats = [
    ...
    def test_format_selection_string_ops(self):     # Anony
        formats = [
    ...
    def test_youtube_format_selection(self):    # Anony
        order = [
    ...
    def test_audio_only_extractor_format_selection(self):   # Anony
    ...
    def test_format_not_available(self):    # Anony
        formats = [
        ...
        self.assertRaises(ExtractorError, ydl.process_ie_result, info_dict.copy()) # not ARV

    def test_format_selection_issue_10083(self):    # Anony
        ...
        ydl.process_ie_result(info_dict.copy())
        self.assertEqual(ydl.downloaded_info_dicts[0]['format_id'], 'video+audio') # not ARV

    def test_invalid_format_specs(self):    # Anony
        def assert_syntax_error(format_spec):
            ydl = YDL({'format': format_spec})
            info_dict = _make_result([{'format_id': 'foo', 'url': TEST_URL}])
            self.assertRaises(SyntaxError, ydl.process_ie_result, info_dict) # not ARV
...
class TestYoutubeDL(unittest.TestCase):
    ...
    def test_prepare_filename(self):    # Anony
        info = {
            'id': '1234',
            'ext': 'mp4',
            'width': None,
    ...
    def test_format_note(self):
        ydl = YoutubeDL()
        self.assertEqual(ydl._format_note({}), '')  # not ARV
    ...
    def test_postprocessors(self):  # Anony
        ...
        self.assertTrue(os.path.exists(filename), '%s doesn\'t exist' % filename)   # not ARV
        self.assertTrue(os.path.exists(audiofile), '%s doesn\'t exist' % audiofile) # not ARV
        os.unlink(filename)
        os.unlink(audiofile)

        run_pp({'keepvideo': False}, SimplePP)
        self.assertFalse(os.path.exists(filename), '%s exists' % filename)      # not ARV
        self.assertTrue(os.path.exists(audiofile), '%s doesn\'t exist' % audiofile) # not ARV
        os.unlink(audiofile)
    ...
    def test_match_filter(self):    # Anony
        class FilterYDL(YDL):
            def __init__(self, *args, **kwargs):
    ...
    def test_playlist_items_selection(self):    # Anony
        entries = [{
            'id': compat_str(i),
            'title': compat_str(i),
            'url': TEST_URL,
    ...
    def test_do_not_override_ie_key_in_url_transparent(self):   # Anony
        ydl = YDL()
    ...
    def test_ignoreerrors_for_playlist_with_url_transparent_iterable_entries(self): # Anony
    ...
    def test_default_times(self):   # Anony
        ...
        self.assertTrue(isinstance(out_info['upload_date'], compat_str))    # not ARV
        self.assertEqual(out_info['upload_date'], '20210911')
        self.assertTrue(isinstance(out_info['release_date'], compat_str))   # not ARV
